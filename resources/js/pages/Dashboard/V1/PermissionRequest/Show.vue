    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-xl font-semibold">Permission Request Details</h1>
            </div>
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                ...
                <Card class="lg:col-span-2">
                        ...
                        <div class="flex gap-2">
                            ...
                            >
                                <Link :href="`/dashboard/permission-requests/${permissionRequest.uuid}/review`">
                                    <ClipboardCheck class="h-4 w-4 mr-2" /> Review
                                </Link>
                            </Button>
                            <Button
                                v-if="permissionRequest.status === 'pending'"
                    ...
                    <CardContent class="space-y-6">
                        <!-- Period -->
                        <div>
                            <h3 class="text-sm font-medium text-muted-foreground mb-2">Period</h3>
                            <div class="flex items-center gap-4">
                        ...
                        <!-- Reason -->
                        <div>
                            <h3 class="text-sm font-medium text-muted-foreground mb-2">Reason</h3>
                        ...
                        <div v-if="permissionRequest.status !== 'pending'" class="border-t pt-6">
                            <h3 class="text-sm font-medium text-muted-foreground mb-4">Review Information</h3>
                            <div class="space-y-3">
                                <div class="flex items-center gap-2">
                                    <User class="h-4 w-4 text-muted-foreground" />
                                    <span class="text-sm">Reviewed by: {{ permissionRequest.reviewer?.name || 'Unknown' }}</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <Clock class="h-4 w-4 text-muted-foreground" />
                                    <span class="text-sm">Reviewed at: {{ permissionRequest.reviewed_at_formatted || '-' }}</span>
                                </div>
                                <div v-if="permissionRequest.review_note">
                                    <p class="text-sm font-medium mb-1">Review Note:</p>
                                ...
                                <div v-if="permissionRequest.rejected_status && permissionRequest.rejected_reason">
                                    <p class="text-sm font-medium mb-1 text-destructive">Rejected Reason:</p>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle class="text-base">Employee</CardTitle>
                    </CardHeader>
                    <CardContent>
                        ...
                        <div class="mt-6 space-y-4 border-t pt-4">
                            <div class="flex justify-between text-sm">
                                <span class="text-muted-foreground">Request Date</span>
